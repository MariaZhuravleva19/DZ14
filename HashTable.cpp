...
public:
	...
	void InsertItem(int key, string value)
	{
		...
		for (; Itr != end(cell); Itr) // нужно знать существует ключ который мы вводим или нет 
		{
			if (Itr->first == key)
			{
				IfKeyExist = true;
				Itr->second = value; // ключ существует, значение заменено
	...
	void RemoveItem(int key)
	{
		...
		auto Itr = begin(cell);
		bool IfKeyExist = false;
		for (; Itr != end(cell); Itr) // нужно знать существует ключ который мы вводим или инет 
		{
			if (Itr->first == key)
			{
				IfKeyExist = true;
				Itr = cell.erase(Itr); // предмет удален 
				break;
			}
		}
